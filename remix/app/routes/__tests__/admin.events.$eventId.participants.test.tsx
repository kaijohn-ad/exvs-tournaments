...
import { loader } from '../admin.events.$eventId.participants';
...
describe('admin event participants route', () => {
	beforeEach(() => {
	...
	describe('loader', () => {
		it('returns event data with players and pairs', async () => {
			...
			const result = await loader({
				...
				context: mockContext,
				request: new Request(`http://localhost/admin/events/${event1.id}/participants`),
			});
		...
		it('returns 404 when event does not exist', async () => {
			await expect(
				loader({
					...
					context: mockContext,
					request: new Request('http://localhost/admin/events/non-existent-event-id/participants'),
				})
			).rejects.toThrow();
		...
		it('returns 400 when eventId is missing', async () => {
			await expect(
				loader({
					params: {},
					context: mockContext,
					request: new Request('http://localhost/admin/events//participants'),
				} as any)
			).rejects.toThrow();
