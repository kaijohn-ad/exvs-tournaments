...
export async function loader({ params, context }: LoaderFunctionArgs) {
	...
	try {
		...
		console.error("[admin.events.$eventId.participants:load] failed", {
			eventId,
...
export const meta: MetaFunction<typeof loader> = ({ data }) => [
	{ title: `${data?.event.name || "イベント"} - 参加者 - 管理 | Boost Bracket` },
	{ name: "description", content: `${data?.event.name || "イベント"}の参加者一覧（管理画面）` },
];

export default function AdminEventParticipantsRoute() {
	...
	return (
		<div className="flex flex-col gap-8">
			...
			<div className="flex gap-3">
				<Link
					to={`/admin/events/${eventId}/entries/players`}
				...
				<Link
					to={`/admin/events/${eventId}/entries/pairs`}
					className="inline-flex items-center rounded-lg border border-blue-200 bg-blue-50 px-4 py-2 text-sm font-medium text-blue-700 transition hover:bg-blue-100"
				>
					ペア管理
				</Link>
