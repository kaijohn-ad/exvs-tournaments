...
export const createMatchesRepositoryD1 = (db: D1Database) => {
	return {
		async listMatches(contextType?: string, contextId?: string): Promise<MatchRecord[]> {
			...
			const result = await (bindings.length > 0 ? stmt.bind(...bindings) : stmt).all<MatchRecord>();
		...
		async createMatch(data: MatchData): Promise<MatchRecord> {
			...
			await db
				.prepare(
					`INSERT INTO matches 
					(id, context, context_id, side_a_type, side_a_pair_id, side_a_player1_id, side_a_player2_id,
		...
		async updateMatch(matchId: string, data: MatchData): Promise<MatchRecord> {
			...
			await db
				.prepare(
					`UPDATE matches 
					SET context = ?, context_id = ?, side_a_type = ?, side_a_pair_id = ?, 
