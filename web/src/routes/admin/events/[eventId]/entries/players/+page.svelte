<script lang="ts">
	...
	let editorPayload = form?.payload ?? data.playersJson;
	...
	$: {
		revokeUrl();
		const blob = new Blob([data.playersJson], { type: 'application/json' });
	...
	$: if (form?.playersJson) {
		const blob = new Blob([form.playersJson], { type: 'application/json' });
		revokeUrl();
		downloadUrl = URL.createObjectURL(blob);
	}
	...
	const resetEditor = () => {
		editorPayload = data.playersJson;
		editorError = null;
	};
...
<svelte:head>
	<title>プレイヤー管理 | Boost Bracket Admin</title>
</svelte:head>
...
<section class="container">
	...
	<section class="card">
		<h2>登録済みプレイヤー ({players.length})</h2>
		{#if flashVisible && form?.message && form?.type === 'success' && form?.source !== 'editor'}
			<div class={`flash ${form.type}`}>{form.message}</div>
		{/if}

		{#if players.length === 0}
			<p class="empty">まだプレイヤーが登録されていません。</p>
		{:else}
			<ul class="list">
				{#each players as player}
					<li class="player-item">
						<form method="POST" action="?/update" class="player-form">
							...
							<label>
								<span>メモ</span>
								<textarea name="note" rows="2">{player.note ?? ''}</textarea>
							</label>
	...
	<section class="card import-export">
		...
		<div class="export">
			<h3>エクスポート</h3>
			<textarea readonly rows="8">{data.playersJson}</textarea>
			<a class="secondary" download={`players-${data.eventId}.json`} href={downloadUrl ?? undefined}
				>JSONをダウンロード</a
			>
		</div>
		<div class="import">
			...
			<p class="helper">※ 空行や無効なエントリはスキップされます。既存プレイヤーは上書きされます。</p>
		</div>
	</section>
